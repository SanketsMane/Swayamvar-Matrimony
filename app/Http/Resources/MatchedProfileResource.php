...
class MatchedProfileResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        if ($this->user != null) {
            $profile_view_resquest_status = ViewProfilePicture::where('user_id', $this->match_id)->where('requested_by', auth()->id())->where('status', 1)->first();
            $gallery_view_resquest_status = ViewGalleryImage::where('user_id', $this->match_id)->where('requested_by', auth()->id())->where('status', 1)->first();
            ...
            return [
                ...
                'marital_status'       => $this->user?->member?->marital_status?->name ?? '',
                ...
                'sub_caste'            => $this->user?->spiritual_backgrounds?->sub_caste?->name ?? "",
                "report_status"        => ReportedUser::where('user_id', $this->match_id)->where('reported_by', auth()->id())->first() ? true : false,
                "shortlist_status"    => Shortlist::where('user_id', $this->match_id)->where('shortlisted_by', auth()->id())->first() ? 1 : 0,
