...
class MemberResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        if ($this->member != null) {
            ...
            $p_view_status = $profile_view_resquest_status->where('status', 1)->first();
            ...
            $g_view_status = $gallery_view_resquest_status->where('status', 1)->first();
            ...
            $shortlist_status = Shortlist::where('user_id', $this->id);
            if ($token) {
                $shortlist_status->where('shortlisted_by', $user->id);
            }
            $is_shortlisted = $shortlist_status->first();

            $do_interest_status = ExpressInterest::where('user_id', $this->id);
            if ($token) {
                $do_interest_status->where('interested_by', $user->id);
            }
            $did_interest = $do_interest_status->first();

            $received_interest_status = ExpressInterest::where('interested_by', $this->id);
            if ($token) {
                $received_interest_status->where('user_id', $user->id);
            }
            $got_interest = $received_interest_status->first();

            $profile_reported_status = ReportedUser::where('user_id', $this->id);
            if ($token) {
                $profile_reported_status->where('reported_by', $user->id);
            }
            $is_reported = $profile_reported_status->first();
            ...
            return [
                ...
                'interest_status'      => ($did_interest ? 'sent interest' : $got_interest) ? 'received interest' : 'no interest',
                ...
                'shortlist_status'     => $is_shortlisted ? 1 : 0,
                ...
                'report_status'        => $is_reported ? true : false,
                'profile_view_resquest_status'   => $p_view_status ? true : false,
                'gallery_view_resquest_status'   => $g_view_status ? true : false,
            ];
        }
    }
}
