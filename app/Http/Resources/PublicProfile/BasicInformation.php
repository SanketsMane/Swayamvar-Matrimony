...
class BasicInformation extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        return [
            ...
            'code'           => $this->code,
            'age'            => $age,
            'religion_id'    => $this->spiritual_backgrounds?->religion_id ? (int)$this->spiritual_backgrounds->religion_id : null,
            'religion'       => $this->spiritual_backgrounds?->religion?->name ?? '',
            'caste_id'       => $this->spiritual_backgrounds?->caste_id ? (int)$this->spiritual_backgrounds->caste_id : null,
            'caste'          => $this->spiritual_backgrounds?->caste?->name ?? '',
            'sub_caste_id'   => $this->spiritual_backgrounds?->sub_caste_id ? (int)$this->spiritual_backgrounds->sub_caste_id : null,
            'date_of_birth'  => !empty($this->member?->birthday) ? Carbon::parse($this->member->birthday)->format('Y-m-d') : '',
            'onbehalf'       => $this->member ? new OnBehalfResource(OnBehalf::find($this->member->on_behalves_id)) : null,
